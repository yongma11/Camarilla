...
def loss_streak_mul(recent_outcomes):
    ...
    return 1.0
# ★ v10.3: 카마릴라 R4 오버레이 기본값 (settings.json으로 덮어쓸 수 있음, bot.py(v10.3)와 동일)
#   - v10.0 → v10.1 변경점:
#     ① 저항선을 R4(coef=0.70, 실제 계수 1.1*0.70=0.77)에서 R3(coef=0.25, 실제 계수
#        1.1*0.25=0.275, 정석 카마릴라 R3 공식과 동일)로 교체 — 17년 SOXL 실데이터
#        검증(집계 통계) 결과 R3가 R4보다 거래수/승률/평균수익/최대손실 전부 개선.
#     ② 변동성 백분위 필터 완전 삭제 — R3 기준 테스트에서 모든 계수 구간에서 필터가
#        좋은 거래까지 같이 잘라내며 성과를 깎는 것으로 확인되어(검증 결과 전부
#        역효과) 폐기. (★ v10.3: 이 결론은 R3에서만 유효했음 — 아래 참고)
#     ③ 1회 진입 한도(cama_max_invest) 신설 — 실제 체결 현실성(슬리피지/유동성) 반영.
#        한도를 넘는 초과분은 카마릴라에 투입하지 않을 뿐, real_cash에서 차감된 적이
#        없으므로 그대로 동파법이 쓸 수 있는 현금으로 남는다 (★ v10.1.1: 같은 금액을
#        cum_profit에도 더하던 이중 계상 버그 제거 — 매수 집중도 폭증의 원인이었음).
#   - v10.1 → v10.2 변경점 (★ R4로 복귀):
#     버그 수정(v10.1.1) 후에도 전체 콤보 MDD가 -51.1%(2022-01-04 → 2022-11-03)로
#     과도했음. 실거래 로그로 귀속 분석한 결과, 이 구간 손실의 82%(-$866,884)가
#     한도($1M) 적용 여부와 무관하게 R3 자체의 거래 특성(2022 SOXL 하락장에서 더 잦은
#     진입·낮은 승률 41.5%)에서 발생한 것으로 확인됨(당시 평균 투입액도 한도 미달).
#     즉 MDD의 주원인은 $1M 캡 메커니즘이 아니라 R3의 공격적 진입 빈도였음.
#     R3가 집계 성과는 더 좋지만 하락장 리스크가 크다고 판단하여, 저항선만 R4
#     (coef=0.50, 실제 계수 1.1*0.50=0.55)로 복귀. $1M 캡과 v10.1.1 버그 수정은
#     그대로 유지(초과분은 real_cash 보유로 동파에 자동 환입, cum_profit 이중계상 없음).
#   - v10.2 → v10.3 변경점 (★ R4 전용 그리드서치 + 변동성 필터 재도입 + 수수료/슬리피지
#     현실화):
#     v10.1에서 폐기한 변동성 필터는 R3 기준 테스트 결과였을 뿐, R4 기준으로는
#     검증한 적이 없었음. $1M 캡을 포함한 콤보엔진으로 SOXL 17년 데이터에 대해
#     coef(0.50~0.80) × 변동성필터(상위 30~5% 회피/없음) × 투입비중 그리드서치 후,
#     상위 후보를 워크포워드(IS 2010~2021-07 / OOS 2021-07~2026-06, 2022년 급락
#     포함)로 재검증한 결과:
#       - coef만 올리고 필터 없음: OOS MDD -42.4% (현재보다 악화)
#       - 필터만 추가(coef 그대로): OOS MDD -32.8% (소폭 개선)
#       - coef=0.70 + 변동성 상위 20% 회피 둘 다 적용(투입비중은 0.70 유지):
#         OOS MDD -34.6%→-29.1%, OOS Calmar 3.55→4.03 — 가장 견고한 개선.
#     → cama_coef를 0.50(정통 R4)에서 0.70으로, vol 필터를 다시 도입(기본 ON,
#     상위 20% 변동성일 회피)으로 변경. coef=0.70은 숫자상 v10.0의 옛 기본값과
#     같지만, 그때는 "R4"로 잘못 라벨링된 미검증 값이었고 지금은 $1M 캡+필터를
#     포함한 그리드서치·워크포워드 검증을 통과한 값이라는 점이 다름.
#     수수료/슬리피지도 현실화: cama_fee_rate 0.05%→0.10%(편도, 왕복 시 ×2),
#     cama_slippage_pct 0.10%→0.15%(편도) — 레버리지 ETF 돌파매매 체결은 동파법의
#     계획된 매수보다 슬리피지가 더 클 것으로 보아 보수적으로 상향.
OVERLAY_DEFAULTS = {
    'overlay_enabled':        False,   # 기본 false = v9.8과 100% 동일 동작 (하위호환)
    'overlay_fraction':       0.70,
    'cama_coef':              0.70,    # ★ v10.3: R4 그리드서치 최적값 (1.1 * 0.70 = 0.77)
    'cama_vol_filter_enabled': True,    # ★ v10.3: 변동성 필터 재도입 (R4 기준 검증)
    'cama_vol_filter_pct':    0.80,    # ★ v10.3: 직전 20일 변동성이 과거 분포 상위 20%면 진입 회피
    'cama_fee_rate':          0.0010,  # ★ v10.3: 0.05%→0.10% (편도, 왕복 적용 시 ×2)
    'cama_slippage_pct':      0.0015,  # ★ v10.3: 0.10%→0.15% (편도, 현실화)
    'cama_max_invest':        1000000.0,  # ★ v10.1: 카마릴라 1회 투입 한도($). 초과분→동파 현금 환입
}
try:
...
# ★ v10.3: 카마릴라 저항선 돌파 신호 계산 (기본 coef=0.70 → R4 그리드서치 최적값.
#   bot.py(v10.3)와 동일 로직, SOXL 고정)
#   변동성 백분위 필터는 v10.1에서 R3 기준으로 삭제됐었으나, R4 기준 그리드서치+워크포워드
#   재검증 결과 coef를 같이 올릴 때만 견고하게 효과가 있는 것으로 확인되어 v10.3에서 재도입.
def compute_camarilla_signal(df, coef=0.70, vol_filter_pct=0.80):
    """df는 SOXL_O/SOXL_H/SOXL_L/SOXL 컬럼 필요.
    resistance[d] = 전일 종가 + 전일 고저폭 * 1.1 * coef  (coef=0.25 → R3, coef=0.50 → R4,
    coef=0.70 → v10.3 R4 그리드서치 최적값)
    signal[d]     = 오늘 고가가 저항선 이상 AND (vol_filter_pct가 None이 아니면) 직전 20일
                    실현변동성의 확장윈도우 백분위가 vol_filter_pct 이하 (상위 X% 변동성일 회피)
    ...
    if vol_filter_pct is not None:
        # ★ v10.3: 직전 20일 일별수익률 변동성의 확장윈도우(최소 60일) 백분위 랭크.
        ret_cc = np.zeros(n)
        ret_cc[1:] = C[1:] / C[:-1] - 1
...
def _camarilla_today_candidate(df, real_cash, overlay_fraction, coef, max_invest=1000000.0,
                                vol_filter_pct=None):
    """오늘(아직 데이터에 없는 다음날) 신규진입 후보가를 어제까지의 실제 OHLC만으로
    계산한다 (look-ahead 없음). ★ v10.1: 1회 투입 한도(max_invest) 반영.
    ★ v10.3: vol_filter_pct가 주어지면, 오늘이 '변동성 상위 X% 회피' 구간에 해당해
    설령 가격이 저항선을 돌파해도 신호가 무시되는 날인지(vol_filter_blocked)도 함께 반환."""
    ...
    out = {
        'resistance': resistance_today, 'has_candidate': False, 'vol_filter_blocked': False,
    }
    if vol_filter_pct is not None:
        C = df['SOXL'].values
        n = len(C)
        ...
        vol_rank = vol20.expanding(min_periods=60).rank(pct=True)
        today_vol_rank = vol_rank.iloc[-1]
        if pd.notna(today_vol_rank):
            out['vol_filter_blocked'] = bool(today_vol_rank > vol_filter_pct)
    if resistance_today > 0 and not np.isnan(resistance_today):
        out['has_candidate'] = True
        raw_amt = overlay_fraction * max(0.0, real_cash)
        out['invest_amt_candidate'] = min(raw_amt, max_invest)
        out['excess_to_dongpa_candidate'] = max(0.0, raw_amt - max_invest)
    return out
def get_overlay_cfg(settings):
    ...
    cfg['cama_max_invest']  = float(cfg.get('cama_max_invest', 1000000.0))  # ★ v10.1
    cfg['cama_vol_filter_enabled'] = bool(cfg.get('cama_vol_filter_enabled', True))  # ★ v10.3
    cfg['cama_vol_filter_pct']     = float(cfg.get('cama_vol_filter_pct', 0.80))     # ★ v10.3
    return cfg
def get_repo():
    g = Github(GH_TOKEN)
...
def auto_sync_engine(df, start_date, init_cap, withdrawals_df=None, overlay_cfg=None):
    ...
    cama_max_invest     = overlay_cfg.get('cama_max_invest', 1000000.0)  # ★ v10.1
    cama_vol_filter_pct = overlay_cfg.get('cama_vol_filter_pct', 0.80) \
        if overlay_cfg.get('cama_vol_filter_enabled', True) else None  # ★ v10.3
    ...
    cama_win_count   = 0
    cama_total_pnl   = 0.0
    cama_excess_to_dongpa = 0.0   # ★ v10.1: 1회 투입한도 초과분 중 동파 real_cash로 남은 누적액(추적용, v10.1.1부터 cum_profit엔 미가산)
    for date, row in sim_df.iterrows():
        ...
        if overlay_enabled and bool(row.get('CamaSignal', False)) and cama_position is None:
            ...
            if entry_raw > 0 and not np.isnan(entry_raw):
                ...
                invest_amt_raw = overlay_fraction * leftover
                invest_amt     = min(invest_amt_raw, cama_max_invest)
                excess         = invest_amt_raw - invest_amt
                if invest_amt > 1e-6:
                    ...
                    full_action_log.append({
                        ...
                        "수익금": "-", "비고": f"신규 투입 ${invest_amt:,.0f}"
                                + (f" (한도초과 ${excess:,.0f} → 동파 현금으로 환입)" if excess > 1e-6 else "")
                    })
                if excess > 1e-6:
                    # ★ v10.1.1 (버그수정): 1회 투입 한도($cama_max_invest) 초과분은 카마릴라에
                    #   넣지 않을 뿐, real_cash에서 원래부터 빠져나간 적이 없다 (위에서 invest_amt만
                    #   차감했음). 즉 "동파법에 투입"은 이미 real_cash 보유 형태로 자동 충족된다.
                    #   ★ v10.1에서는 여기서 cum_profit에도 같은 금액을 추가했었는데, 이는 같은
                    #   돈을 (1) real_cash에 실제로 남겨두면서 (2) 가상자본 계산식에도 "동파 실현수익"
                    #   인 것처럼 또 더하는 이중 계상 버그였다. cum_profit은 영구 누적(절대 감소 없음)
                    #   되어 슬롯크기(virtual capital/MAX_SLOTS)를 계좌가 커질수록 기하급수적으로
                    #   부풀렸고, 그 결과 실전 매수 시 `min(real_cash, slot_size)`가 거의 항상
                    #   real_cash 쪽에서 막혀 — 7슬롯 분산 대신 가용 현금 전액을 단일 슬롯에 몰아넣는
                    #   사실상 '몰빵' 매매로 변질되었다 (집중도 폭증 확인됨). v10.1.1에서 cum_profit
                    #   가산을 제거해 슬롯 크기가 동파 자신의 실현손익에만 반응하도록 원복했다.
                    cama_excess_to_dongpa += excess
    ...
    if overlay_enabled:
        ...
        cama_info['excess_to_dongpa'] = cama_excess_to_dongpa  # ★ v10.1
        ...
        if cama_position is not None:
            ...
            today.update(_camarilla_today_candidate(
                df, real_cash, overlay_fraction, cama_coef, max_invest=cama_max_invest,
                vol_filter_pct=cama_vol_filter_pct))
...
def run_backtest_fixed(df, start_date, end_date, init_cap,
                        ...
                        cama_coef=0.70, cama_max_invest=1000000.0,
                        cama_fee_rate=0.0010, cama_slippage_pct=0.0015,
                        cama_vol_filter_pct=0.80,
                        cama_tax_combined=False):
    """★ v9.8 원본 로직(세금/수수료 트랜치 스케줄링 포함) + ★ v10.1 카마릴라
    ...
    cama_coef=0.70 (기본값, ★ v10.3): R4 부근 저항선 (1.1*0.70 = 0.77). v10.2까지는
    coef=0.50(정통 R4, 1.1*0.50=0.55)·필터 없음이 기본값이었음. R3(coef=0.25,
    1.1*0.25=0.275)는 17년 SOXL 집계 통계상 거래수/승률/평균수익/최대손실이 R4보다
    더 좋았으나, 2022 SOXL 하락장 구간(2022-01-04~2022-11-03)에서 더 잦은 진입과
    낮은 승률(41.5%)로 콤보 전체 MDD -51.1% 중 82%(-$866,884)를 카마릴라 자체
    손실만으로 유발한 것이 실거래 로그 귀속 분석으로 확인됨(당시 $1M 캡은 23%만
    적용되어 캡이 원인이 아니었음). 하락장 리스크를 줄이기 위해 v10.2에서 저항선을
    R4로 복귀.

    ★ v10.3 — R4 전용 그리드서치 + 변동성 필터 재도입: v10.1에서 폐기한 변동성
    필터는 R3 기준 테스트 결과였을 뿐 R4에서는 검증된 적이 없었다. $1M 캡을 포함한
    콤보엔진으로 SOXL 17년 데이터에 coef(0.50~0.80) × 변동성필터(상위 30%~5%
    회피/없음) × 투입비중 그리드서치 후, 워크포워드(IS 2010~2021-07 / OOS
    2021-07~2026-06, 2022년 급락 포함)로 재검증한 결과: coef만 올리고 필터 없음
    → OOS MDD가 -42.4%로 오히려 악화. 필터만 추가(coef는 그대로 0.50) → OOS MDD
    -32.8%로 소폭 개선에 그침. coef=0.70 + 변동성 상위 20% 회피를 함께 적용(투입
    비중 0.70 그대로) → OOS MDD -34.6%→-29.1%, OOS Calmar 3.55→4.03 으로 가장
    견고한 개선. 즉 coef를 올리는 것과 필터를 추가하는 것은 따로따로는 효과가
    약하거나 역효과이고, **둘을 함께 적용했을 때만** 과적합 없이(워크포워드 검증)
    개선됨. coef=0.70은 숫자상 v10.0의 옛 "R4" 기본값과 같지만, 그때는 미검증
    상태로 잘못 R4라 라벨링된 값이었고 지금은 $1M 캡 + 필터를 포함한 그리드서치·
    워크포워드 검증을 통과한 값이라는 점이 다르다.

    cama_vol_filter_pct=0.80 (★ v10.3 신규): 직전 20일 실현변동성의 확장윈도우
    백분위가 80%를 넘는 날(=과거 대비 변동성 상위 20% 구간)은 돌파 신호가 있어도
    진입하지 않는다. None을 넘기면 필터를 끈다(v10.1~v10.2 동작과 동일).

    cama_fee_rate=0.0010 / cama_slippage_pct=0.0015 (★ v10.3, 현실화): 기존
    0.0005(0.05%)/0.0010(0.10%)에서 매수 수수료 0.1%(편도, 왕복 적용 시 ×2),
    슬리피지 0.15%(편도)로 상향. 레버리지 ETF 돌파매매는 저항선 통과 직후 호가가
    빠르게 움직이는 구간에서 체결되므로, 동파법의 계획된 매수보다 체결 비용이
    더 클 것으로 보아 보수적으로 반영함.

    cama_max_invest=1,000,000.0 (★ v10.1 신규): 카마릴라 1회 진입 한도($). 실제 체결
    현실성(대형 포지션의 슬리피지/유동성 제약)을 반영하기 위해 도입. overlay_fraction *
    real_cash 가 한도를 넘으면 한도까지만 카마릴라에 투입하고, 초과분은 카마릴라에
    넣지 않는다 — 그 금액은 처음부터 invest_amt로 차감되지 않았으므로 real_cash에
    그대로 남아 다음 동파 매수에 즉시 쓰인다 ("동파법에 투입"은 이 형태로 충족됨).
    ★ v10.1.1 버그수정: 초기 버전(v10.1)은 여기서 초과분을 cum_profit(동파 복리
    가상자본 계산식의 입력)에도 추가했었다. 이는 같은 돈을 ①real_cash 보유 ②동파
    실현수익 가산으로 이중 계상하는 버그였고, cum_profit은 영구 누적(절대 감소 없음)
    이라 계좌가 커질수록(특히 한도를 자주 초과하는 2021년 이후) 슬롯 크기(virtual
    capital/MAX_SLOTS)를 기하급수적으로 부풀렸다. 그 결과 실제 매수 시
    `amt = min(real_cash, slot_size*qs_mul*ls_mul)`이 거의 항상 real_cash 쪽에서
    막혀 — 본래 최대 7슬롯(Safe)/4슬롯(Offense)으로 분산해야 할 매수가 가용 현금
    전액을 단일 슬롯에 몰아넣는 사실상 '몰빵' 매매로 변질됐다(실데이터 검증: 매수
    1건당 총자산 대비 투입비중이 2010~2020년 평균 5~14%에서 2021년 이후 평균
    17~42%, 최대 99~100%로 폭증). v10.1.1에서 cum_profit 가산을 제거해 슬롯
    크기가 동파 자신의 실현손익에만 반응하도록 복구했다.

    cama_tax_combined=True (기본 False, 기존 동작 보존):
      카마릴라 손익도 과세 대상에 포함한다. 단, '공제 1회'는 실제 한국 양도세처럼
      동파+카마릴라 합산 기준(연 1회 $tax_deduction_usd)으로 계산하고, 그렇게 나온
      세금 총액은 그 해 두 전략이 만든 '실현이익 비중'대로 나눠서 부과한다(손실
      쪽에는 책임을 묻지 않음). 동파가 낸 세금(total_tax_paid)만 동파 자신의
      가상자본(슬롯 크기, virtual capital) 계산에 들어가고, 카마릴라가 낸 세금
      (total_tax_paid_cama)은 별도 변수로 분리해 동파 슬롯 크기에 영향을 주지
      않는다 — 동파 쪽 cum_profit/cum_loss에 카마릴라 손익을 안 섞는 기존 '회계
      분리' 원칙과 일관되게, 세금 쪽도 동일하게 분리한 것이다. (v1 패치는 이 분리
      없이 total_tax_paid 하나에 합쳤다가, 카마릴라 세금이 동파 슬롯 크기까지
      깎아먹는 의도치 않은 부작용이 있어 v2에서 분리함.)
      카마릴라 수익이 그대로 real_cash에 들어가 다음 카마릴라 베팅 크기를 키우는
      구조이므로, 세금을 매기면 이후 회차의 실제 손익 자체도 줄어든다 — 따라서
      연도별 표에 사후적으로 22%를 곱하는 방식은 부정확하며, 반드시 이 옵션으로
      처음부터 다시 시뮬레이션해야 한다."""
    ...
    annual_realized_cama_tax = 0.0
    total_tax_paid = 0.0
    total_tax_paid_cama = 0.0
    last_year_seen = None
    tax_log = []
    yearly_realized_log = {}
    yearly_fee_log = {}
    ...
    for date, row in sim_df.iterrows():
        ...
        if overlay_enabled and cama_position is not None:
            ...
            real_cash += proceeds
            cama_total_pnl += pnl
            if include_tax and cama_tax_combined:
                annual_realized_cama_tax += pnl  # ★ 동파의 annual_realized_tax와는 별도 버킷
            yearly_cama_pnl_log[cur_year] = yearly_cama_pnl_log.get(cur_year, 0.0) + pnl
            cama_trade_count += 1
            if pnl > 0:
                cama_win_count += 1
            held_days = (date.date() - cama_position['entry_date'].date()).days
            est_shares = cama_position['invest_amt'] / entry_eff if entry_eff > 0 else 0.0
            cama_log.append({
                "진입일": cama_position['entry_date'].date(), "청산일": date.date(),
                "보유일": held_days,
                "매수가(저항선)": f"${cama_position['entry_raw']:.2f}",
                "매수가(체결)": f"${entry_eff:.2f}",
                "매도가(익일시가)": f"${exit_eff:.2f}" if not np.isnan(exit_eff) else "-",
                "수량(추정)": f"{est_shares:,.1f}",
        ...
        if last_year_seen is not None and cur_year != last_year_seen:
            ...
            if include_tax:
                # ★ 공제($tax_deduction_usd)는 동파+카마릴라 합산 1회만 적용(실제 한국 양도세와
                #   동일하게 종목 구분 없이 연 1회) — 단, 그렇게 산출된 세금은 그 해에 각자 만든
                #   '실현이익 비중'대로 나눠서 부과한다(손실을 낸 쪽에는 책임을 묻지 않음).
                combined_realized_tax = annual_realized_tax + annual_realized_cama_tax
                annual_tax = max(0.0, combined_realized_tax - tax_deduction_usd) * tax_rate
                pos_dongpa = max(0.0, annual_realized_tax)
                pos_cama = max(0.0, annual_realized_cama_tax)
                pos_sum = pos_dongpa + pos_cama
                if pos_sum > 1e-9:
                    dongpa_share = pos_dongpa / pos_sum
                else:
                    dongpa_share = 1.0
                cama_share = 1.0 - dongpa_share
                if annual_tax > 0:
                    for entry in tax_tranches_def:
                        ...
                        if yoff == -1:
                            ...
                            if actual > 0:
                                real_cash -= actual
                                total_tax_paid += actual * dongpa_share
                                total_tax_paid_cama += actual * cama_share
                            ...
                            if remaining > 1e-6:
                                pending_tranches.append({
                                    ...
                                    'dongpa_share': dongpa_share, 'cama_share': cama_share,
                                })
                        else:
                            pending_tranches.append({
        ...
        if overlay_enabled and bool(row.get('CamaSignal', False)) and cama_position is None:
            ...
            if not np.isnan(next_open) and entry_raw > 0:
                ...
                if excess > 1e-6:
                    # ★ v10.1.1 (버그수정): 한도 초과분은 카마릴라에 안 넣을 뿐, real_cash에서
                    #   빠져나간 적이 없다(위에서 invest_amt만 차감). "동파법에 투입"은 real_cash
                    #   보유로 이미 충족된다. v10.1은 여기서 cum_profit에도 같은 금액을 더해
                    #   같은 돈을 (1)real_cash 보유 + (2)동파 가상자본 가산으로 이중 계상했고,
                    #   cum_profit은 영구 누적(감소 없음)이라 계좌가 커질수록 슬롯크기를 기하급수적
                    #   으로 부풀려 실제 매수가 `min(real_cash, slot_size)`에서 거의 항상 real_cash
                    #   쪽 한도에 걸려 7슬롯 분산 대신 가용 현금 전액을 단일 슬롯에 몰아넣는 결과를
                    #   낳았다(매수 1건당 자산 대비 투입비중이 2021년 이후 평균 70~100%로 폭증,
                    #   실데이터 검증으로 확인). v10.1.1에서 cum_profit 가산을 제거해 슬롯 크기가
                    #   동파 자신의 실현손익에만 반응하도록 복구했다.
                    cama_excess_to_dongpa += excess
                    yearly_cama_excess_log[cur_year] = yearly_cama_excess_log.get(cur_year, 0.0) + excess
        ...
        for tranche in pending_tranches:
            ...
            d_share = tranche.get('dongpa_share', 1.0)
            c_share = tranche.get('cama_share', 0.0)
            if past_force:
                actual = remaining
                real_cash -= actual
                total_tax_paid += actual * d_share
                total_tax_paid_cama += actual * c_share
                tranche['paid'] += actual
    ...
    pending_unrealized = (max(0.0, (annual_realized_tax + annual_realized_cama_tax) - tax_deduction_usd) * tax_rate
    ...
    if not res_df.empty:
        ...
        metrics = {
            ...
            'total_tax_paid': total_tax_paid, 'total_tax_paid_cama': total_tax_paid_cama,
            ...
            'cama_excess_to_dongpa': cama_excess_to_dongpa,  # ★ v10.1
    ...
    for yr in res_df.index.year.unique():
        ...
        yearly_stats.append({
            ...
            "카마릴라한도초과→동파": yearly_cama_excess_log.get(yr, 0.0),  # ★ v10.1
        })
...
def main():
    ...
    with tab_trade:
        with st.sidebar:
            ...
            cama_coef = st.slider(
                "저항선 계수 (coef, 0.25=R3 / 0.50=R4 정통 / 0.70=v10.3 그리드서치 추천)", 0.10, 2.00,
                saved_overlay_cfg['cama_coef'], 0.05,
                disabled=not overlay_enabled)
            cama_vol_filter_enabled = st.checkbox(
                "변동성 필터 사용 (★ v10.3 재도입)", value=saved_overlay_cfg['cama_vol_filter_enabled'],
                disabled=not overlay_enabled,
                help="직전 20일 실현변동성이 과거 분포 상위 X%(아래 슬라이더)에 해당하는 날은 돌파 신호가 있어도 진입하지 않습니다. R4 전용 그리드서치+워크포워드 검증 결과 coef=0.70과 함께 적용할 때만 견고하게 MDD를 낮춥니다(coef만 올리면 오히려 악화).")
            cama_vol_filter_pct = st.slider(
                "변동성 필터 임계값 (상위 X% 회피)", 0.50, 0.99,
                saved_overlay_cfg['cama_vol_filter_pct'], 0.05,
                disabled=not overlay_enabled or not cama_vol_filter_enabled,
                help="0.80 = 변동성이 과거 분포 상위 20%에 해당하는 날은 진입 회피 (그리드서치 추천값).")
            cama_max_invest = st.number_input(
                "카마릴라 1회 투입 한도 ($)", value=saved_overlay_cfg['cama_max_invest'], step=50000.0,
                disabled=not overlay_enabled,
                help="이 한도를 넘는 금액은 카마릴라에 투입하지 않습니다. 처음부터 차감된 적이 없으므로 동파법 real_cash(실제 현금)로 그대로 남아 다음 동파 매수에 즉시 쓰입니다 (v10.1.1부터 가상자본/cum_profit에는 가산하지 않음 — 이중계상 버그 수정).")
            cama_fee_rate = st.number_input(
                "왕복 수수료율 (편도)", value=saved_overlay_cfg['cama_fee_rate'], step=0.0001, format="%.4f",
                disabled=not overlay_enabled,
                help="편도 수수료율. 왕복(매수+매도) 시 ×2로 적용됩니다. ★ v10.3: 0.05%→0.10%로 현실화(매수 0.1%).")
            cama_slippage_pct = st.number_input(
                "슬리피지율 (편도)", value=saved_overlay_cfg['cama_slippage_pct'], step=0.0001, format="%.4f",
                disabled=not overlay_enabled,
                help="★ v10.3: 0.10%→0.15%로 현실화 (레버리지 ETF 돌파매매 체결 특성 반영).")
            st.caption("settings.json에 저장되어 다음 실제 매매 봇(bot.py v10.3) 실행 때도 그대로 반영됩니다.")
            st.markdown("---")
            if not offline_mode:
                if st.button("🔄 설정 변경 및 재동기화", type="primary"):
                    new_settings = {
                        ...
                        "cama_coef":           cama_coef,
                        "cama_vol_filter_enabled": cama_vol_filter_enabled,
                        "cama_vol_filter_pct":     cama_vol_filter_pct,
                        "cama_max_invest":     cama_max_invest,
        ...
        if not offline_mode:
            ...
            if curr_cama.get('enabled'):
                ...
                if cama_today.get('has_open_position'):
                    ...
                    """, unsafe_allow_html=True)
                elif cama_today.get('has_candidate') and cama_today.get('vol_filter_blocked'):
                    # ★ v10.3: 변동성 상위 X% 구간 — 돌파해도 신호 무시(진입 안 함)
                    st.markdown(f"""
                    <div class="st-card" style="border-left: 5px solid #9e9e9e;">
                        🧨 <strong>카마릴라 R4 — 변동성 필터로 오늘 진입 보류</strong>
                        &nbsp;&nbsp;|&nbsp;&nbsp;
                        <span class="badge-cama-none">저항선 ${cama_today['resistance']:.2f} 돌파해도 진입하지 않음 (오늘 변동성이 과거 분포 상위 {(1-saved_overlay_cfg.get('cama_vol_filter_pct', 0.80))*100:.0f}% 구간)</span>
                    </div>
                    """, unsafe_allow_html=True)
                elif cama_today.get('has_candidate'):
                    excess_note = ""
                    if cama_today.get('excess_to_dongpa_candidate', 0.0) > 1e-6:
                        excess_note = f" (한도초과 ${cama_today['excess_to_dongpa_candidate']:,.0f} → 동파 현금으로 그대로 보유)"
                    st.markdown(f"""
                    <div class="st-card" style="border-left: 5px solid #1a73e8;">
                        🧨 <strong>카마릴라 R4 — 신규진입 후보</strong>
                        &nbsp;&nbsp;|&nbsp;&nbsp;
                        <span class="badge-cama-cand">저항선 ${cama_today['resistance']:.2f} (오늘 고가 돌파 시 진입), 예상 투입금 ${cama_today.get('invest_amt_candidate', 0.0):,.0f}{excess_note}</span>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.markdown(f"""
        ...
        if offline_mode:
            ...
            with st.expander("🧨 카마릴라 R4 오버레이 (백테스트)", expanded=False):
                ...
                bt_cama_coef = st.slider(
                    "저항선 계수 (coef, 0.25=R3 / 0.50=R4 정통 / 0.70=v10.3 그리드서치 추천)", 0.10, 2.00,
                    OVERLAY_DEFAULTS['cama_coef'], 0.05,
                    disabled=not bt_overlay_enabled, key="bt_cama_coef")
                bt_cama_vol_filter_enabled = st.checkbox(
                    "변동성 필터 사용 (★ v10.3 재도입)", value=OVERLAY_DEFAULTS['cama_vol_filter_enabled'],
                    disabled=not bt_overlay_enabled, key="bt_vol_filter_enabled")
                bt_cama_vol_filter_pct = st.slider(
                    "변동성 필터 임계값 (상위 X% 회피)", 0.50, 0.99,
                    OVERLAY_DEFAULTS['cama_vol_filter_pct'], 0.05,
                    disabled=not bt_overlay_enabled or not bt_cama_vol_filter_enabled,
                    key="bt_vol_filter_pct")
                bt_cama_max_invest = st.number_input(
                    "카마릴라 1회 투입 한도 ($)", value=OVERLAY_DEFAULTS['cama_max_invest'], step=50000.0,
                    disabled=not bt_overlay_enabled, key="bt_cama_max_invest",
                    help="이 한도를 넘는 금액은 카마릴라에 투입하지 않습니다. 처음부터 차감된 적이 없으므로 동파법 real_cash(실제 현금)로 그대로 남아 다음 동파 매수에 즉시 쓰입니다 (v10.1.1부터 가상자본/cum_profit에는 가산하지 않음 — 이중계상 버그 수정).")
                ...
                bt_cama_tax_combined = st.checkbox(
                    "💰 카마릴라 손익도 동파와 합산 과세 (세후 비교용)",
                    value=False, disabled=not (bt_overlay_enabled and inc_tax),
                    key="bt_cama_tax_combined",
                    help="동파 단독(세후) vs 오버레이를 동일 기준으로 비교하려면 켜세요. "
                         "'양도세 적용' 체크가 켜져 있어야 동작합니다. 공제($1,786)는 동파+"
                         "카마릴라 합산 1회만 적용되고, 세금은 그 해 실현이익 비중대로 나눠 "
                         "부과됩니다 — 동파가 낸 몫만 동파 슬롯 크기(가상자본)에 영향을 주고, "
                         "카마릴라가 낸 몫은 분리돼 동파 쪽을 깎아먹지 않습니다. 카마릴라 손익은 "
                         "real_cash로 들어가 다음 베팅 크기를 키우는 구조라, 세금을 매기면 "
                         "이후 회차 손익 자체도 줄어들어 단순 사후 22% 차감과는 결과가 다릅니다.")
                if bt_overlay_enabled and bt_cama_tax_combined and not inc_tax:
                    st.warning("⚠️ '양도세 적용 (시뮬)' 체크박스를 먼저 켜야 합산 과세가 반영됩니다.")
            if st.button("🚀 분석 실행"):
                with st.spinner("분석 중..."):
                    res, metrics, df_yearly, df_debug, df_cama = run_backtest_fixed(
                        ...
                        cama_coef=bt_cama_coef, cama_max_invest=bt_cama_max_invest,
                        cama_fee_rate=bt_cama_fee_rate, cama_slippage_pct=bt_cama_slippage_pct,
                        cama_vol_filter_pct=(bt_cama_vol_filter_pct if bt_cama_vol_filter_enabled else None),
                        cama_tax_combined=bt_cama_tax_combined)
                    if res is not None:
                        ...
                        if metrics.get('overlay_enabled'):
                            ...
                            oc4.metric("한도초과→동파 현금 보유액",
                                       f"${metrics.get('cama_excess_to_dongpa', 0.0):,.0f}",
                                       help="카마릴라 1회 투입 한도를 넘어 카마릴라에 투입되지 않고 동파법 real_cash로 남은 누적액 (v10.1.1부터 가상자본/cum_profit에는 가산하지 않음)")
                        ...
                        if metrics.get('overlay_enabled') and df_cama is not None and not df_cama.empty:
                            with st.expander("🧨 카마릴라 거래 로그", expanded=False):
                                n_win = (df_cama['결과'] == '✅ 승').sum() if '결과' in df_cama.columns else 0
                                n_lose = len(df_cama) - n_win
                                st.caption(f"총 카마릴라 거래: **{len(df_cama):,}건** "
                                           f"(승 {n_win:,} / 패 {n_lose:,})")
                                df_cama_view = df_cama.sort_values('진입일', ascending=False).reset_index(drop=True)
                                st.dataframe(df_cama_view, use_container_width=True, height=400, hide_index=True)
                    else:
                        st.error("데이터 부족")
    with tab_logic:
        ...
        * **특징:** 대응 중심의 변동성 돌파 & 추세 추종 하이브리드.
        ...
        ---
        ### 4. ★ v10.3 — 카마릴라 R4 돌파매매 오버레이 (그리드서치 coef+변동성필터 + $1M 투입한도)
        ...
        3. **③ 신규 진입** — 동파법이 다 쓰고 남은 real_cash의 `overlay_fraction`(기본 70%)만큼, 오늘 Camarilla R4 저항선 돌파 신호가 있고 변동성 필터에도 걸리지 않으면 신규 진입 (단일 슬롯, 1회 투입 한도 `cama_max_invest` 적용).

        저항선 공식: `resistance[D] = 전일종가 + (전일고가-전일저가) × 1.1 × coef` (coef=0.25 → R3, coef=0.50 → 정통 R4, **coef=0.70 → v10.3 기본값**), 신호는 `당일고가 ≥ resistance` AND (필터 켜짐 시) `직전 20일 변동성의 확장윈도우 백분위 ≤ cama_vol_filter_pct`. 진입가는 시가가 저항선 이상이면 시가, 아니면 저항선가. 청산은 무조건 다음 거래일 시가.

        `overlay_enabled=False`(기본값)이면 ①③ 스텝이 완전히 스킵되어 v9.8과 100% 동일하게 동작합니다 — 하위호환이 보장됩니다.

        ★ v10.1 신규: `overlay_fraction × real_cash`가 `cama_max_invest`(기본 $1,000,000)를 넘으면, 넘는 만큼은 카마릴라에 투입하지 않습니다. 이 초과분은 처음부터 invest_amt로 차감되지 않았으므로 real_cash에 그대로 남아 다음 동파 매수에 즉시 쓰입니다 — "동파법에 투입"은 이 형태로 충족됩니다.

        ★ v10.1.1 버그수정: 초기 버전(v10.1)은 이 초과분을 동파법의 복리 가상자본(`cum_profit`)에도 똑같이 더했습니다. 같은 돈을 ①real_cash 보유 ②동파 실현수익 가산으로 이중 계상한 것인데, `cum_profit`은 영구 누적(절대 감소 없음)이라 계좌가 커질수록(특히 한도를 자주 넘는 2021년 이후) 슬롯 크기(virtual capital/MAX_SLOTS)를 기하급수적으로 부풀렸습니다. 그 결과 실제 매수 시 `min(real_cash, slot_size)`가 거의 항상 real_cash 쪽에서 막혀 — 최대 7슬롯(Safe)/4슬롯(Offense) 분산 대신 가용 현금 전액을 단일 슬롯에 몰아넣는 사실상 '몰빵' 매매로 변질됐습니다(실데이터 검증: 매수 1건당 총자산 대비 투입비중이 2010~2020년 평균 5~14%에서 2021년 이후 평균 17~42%, 최대 99~100%로 폭증). v10.1.1에서 `cum_profit` 가산을 제거해 슬롯 크기가 동파 자신의 실현손익에만 반응하도록 복구했습니다.

        ★ v10.2 — R4로 복귀: 버그 수정 후에도 콤보 전체 MDD가 -51.1%(2022-01-04 → 2022-11-03)로 과도했습니다. 실거래 로그 귀속 분석 결과, 이 구간 손실의 82%(-$866,884)는 $1M 캡 적용 여부와 무관하게 R3 자체의 거래 특성(2022 SOXL 하락장에서 더 잦은 진입, 승률 41.5%)에서 발생했음을 확인했습니다(당시 평균 투입액도 한도 미달 — 캡은 원인이 아니었음). R3는 17년 집계 통계상 거래수/승률/평균수익/최대손실이 R4보다 더 좋았지만, 하락장에서의 리스크가 커서 저항선만 R4(coef=0.50)로 복귀했습니다. $1M 캡과 v10.1.1 버그 수정은 그대로 유지됩니다.

        ★ v10.3 — R4 전용 그리드서치 + 변동성 필터 재도입 + 수수료/슬리피지 현실화: v10.1에서 폐기한 변동성 필터는 R3 기준 테스트 결과였을 뿐, R4에서는 검증된 적이 없었습니다. $1M 캡을 포함한 콤보엔진으로 SOXL 17년 데이터에 대해 `coef`(0.50~0.80) × 변동성필터(상위 30%~5% 회피/없음) × 투입비중을 그리드서치한 뒤, 워크포워드(IS 2010~2021-07 / OOS 2021-07~2026-06, 2022년 급락 포함)로 재검증했습니다. coef만 올리고 필터 없음 → OOS MDD가 -42.4%로 오히려 악화. 필터만 추가(coef는 그대로) → OOS MDD -32.8%로 소폭 개선에 그침. **coef=0.70 + 변동성 상위 20% 회피를 함께 적용**(투입비중 0.70 유지) → OOS MDD -34.6%→-29.1%, OOS Calmar 3.55→4.03으로 가장 견고한 개선이 확인됐습니다. 즉 두 레버리지를 따로 쓰면 효과가 약하거나 역효과이고, **함께 적용했을 때만** 과적합 없이 개선됩니다. `cama_coef` 기본값을 0.70으로, `cama_vol_filter_enabled`/`cama_vol_filter_pct`(기본 0.80)를 신설해 기본 ON으로 변경했습니다. coef=0.70은 숫자상 v10.0의 옛 "R4" 기본값과 같지만, 그때는 미검증 상태로 잘못 라벨링된 값이었고 지금은 $1M 캡+필터를 포함한 그리드서치·워크포워드 검증을 통과한 값이라는 점이 다릅니다. 아울러 수수료·슬리피지도 현실화했습니다 — `cama_fee_rate` 0.05%→0.10%(편도, 왕복 적용 시 ×2 = 매수 0.1%/매도 0.1%), `cama_slippage_pct` 0.10%→0.15%(편도). 레버리지 ETF 돌파매매는 저항선 통과 직후 호가가 빠르게 움직이는 구간에서 체결되므로, 동파법의 계획된 매수보다 체결 비용이 더 클 것으로 보아 보수적으로 반영했습니다.

        settings.json의 `overlay_enabled`/`overlay_fraction`/`cama_coef`/`cama_vol_filter_enabled`/`cama_vol_filter_pct`/`cama_max_invest`/`cama_fee_rate`/`cama_slippage_pct` 키는 **bot.py(v10.3)와 공유**되므로, 사이드바에서 토글하면 다음 실제 매매 봇 실행 때도 즉시 반영됩니다 (Single Source of Truth).

        카마릴라는 수수료·슬리피지(왕복)만 반영하고 **세금은 시뮬레이션하지 않습니다** — 실제 양도세는 동파법과 마찬가지로 TaxWithdrawals 시트로 수동 추적합니다. RESET_CYCLE 가상자본 계산에도 카마릴라 손익(승패) 자체나 한도초과분 자체는 섞지 않습니다(회계 분리) — 한도초과분은 real_cash 보유로만 동파에 기여합니다.
        """)
